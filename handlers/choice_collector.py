...
def change_state(message: Message) -> None:
    """
    Asking user to choose in which city he/she prefers to search hotels.
    Confirming information about amount of people in every room
    :param message: User's decision to change the amount of rooms needed
    """
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        rooms_amount = data['rooms_amount']
    if message.text.strip().lower() in ['да', 'д', 'ага', 'yes', 'y']:
...
def price_range(message: Message) -> None:
    ...
    :param message: the range of the hotel's price
    """
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        try:
            ans = message.text.strip().split('-')
...
def confirm_data(message: Message) -> None:
    ...
    if message.text.lower() in ['да', 'д', 'ага', 'yes', 'y']:
        ...
        if location_info_result.get('entities') != []:
