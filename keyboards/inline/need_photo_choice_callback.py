...
def need_photo_choice_callback(call: CallbackQuery):
    with bot.retrieve_data(call.from_user.id) as data:
        if call.data == "да":
            ...
            # bot.register_next_step_handler(call.message, get_hotel_photo)
        else:
            data['hotel_photo'] = {
                'need_photo': False
            }
            if data.get('search').get('mode') in ['lowprice', 'highprice']:
                bot.send_message(call.from_user.id, f"{print_data(data=data)}Верно?")
